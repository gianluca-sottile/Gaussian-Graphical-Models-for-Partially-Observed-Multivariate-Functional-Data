...
for (iconfig in seq_len(nrow(configs))) {
  ...
  storage$theta_err_mat_5 <- storage$theta_err_mat_4 <- storage$theta_err_mat_3 <- storage$theta_err_mat_2 <- storage$theta_err_mat
  storage$auc_theta_vec_5 <- storage$auc_theta_vec_4 <- storage$auc_theta_vec_3 <- storage$auc_theta_vec_2 <- storage$auc_theta_vec
  storage$curve_err_mat_5 <- storage$curve_err_mat_4 <- storage$curve_err_mat_3 <- storage$curve_err_mat_2 <- storage$curve_err_mat
  storage$comp_time_vec_5 <- storage$comp_time_vec_4 <-storage$comp_time_vec_3 <- storage$comp_time_vec_2 <- storage$comp_time_vec
  ...
  for (isim in seq_len(n_sim)) {
    ...
    pofggm_rho <- vector("list", length = 5)
    ...
    ALPHA_MAT <- sapply(fit_pofggm$fit_list, \(fit) fit$alpha_opt)
    alpha_mean <- mean(ALPHA_MAT[,1])
    ...
    pofggm_rho[[1]] <- fit_pofggm$fit_list
    rho_max <- fit_pofggm$fit_list$rho_max
    ...
    pofggm_rho[[2]] <- fit_pofggm_2$fit_list
    ...
    pofggm_rho[[3]] <- fit_pofggm_3$fit_list
    ...
    if (inherits(fit_pofggm_4, "try-error")) {
      warning("POFGGM path failed at simulation ", isim)
      next
    }
    
    pofggm_rho[[4]] <- fit_pofggm_4$fit_list
    rho_max_4 <- fit_pofggm_4$rho_max
    grid_rho_4 <- fit_pofggm_4$grid_rho
    storage$comp_time_vec_4[isim] <- fit_pofggm_4$elapsed
    ...
    if (inherits(fit_pofggm_5, "try-error")) {
      warning("POFGGM path failed at simulation ", isim)
      next
    }
    
    pofggm_rho[[5]] <- fit_pofggm_5$fit_list
    rho_max_5 <- fit_pofggm_5$rho_max
    grid_rho_5 <- fit_pofggm_5$grid_rho
    storage$comp_time_vec_5[isim] <- fit_pofggm_5$elapsed
    ...
    if (plot_it) {
      ...
      matlines(tp, pofggm_rho[[imod]][[1]]$Ximputed[, id_plot, 1], col = "red2", lty = 2)
      if (length(pofggm_rho[[imod]]) >= 6)  matlines(tp, pofggm_rho[[imod]][[6]]$Ximputed[, id_plot, 1], col = "blue2", lty = 2)
      if (length(pofggm_rho[[imod]]) >= 11) matlines(tp, pofggm_rho[[imod]][[11]]$Ximputed[, id_plot, 1], col = "green2", lty = 2)
      if (length(pofggm_rho[[imod]]) >= 16) matlines(tp, pofggm_rho[[imod]][[16]]$Ximputed[, id_plot, 1], col = "purple", lty = 2)
      if (length(pofggm_rho[[imod]]) >= 21) matlines(tp, pofggm_rho[[imod]][[21]]$Ximputed[, id_plot, 1], col = "yellow2", lty = 2)
    ...
    theta_err_pofggm_3 <- compute_theta_error_path_pofggm(
      fit_list = pofggm_rho[[3]],
      theta_true_array = out_rPar$Tht,
      normalize = "fro"
    )
    storage$theta_err_mat_3[, isim] <- theta_err_pofggm_3
    
    theta_err_pofggm_4 <- compute_theta_error_path_pofggm(
      fit_list = pofggm_rho[[4]],
      theta_true_array = out_rPar$Tht,
      normalize = "fro"
    )
    storage$theta_err_mat_4[, isim] <- theta_err_pofggm_4
    
    theta_err_pofggm_5 <- compute_theta_error_path_pofggm(
      fit_list = pofggm_rho[[5]],
      theta_true_array = out_rPar$Tht,
      normalize = "fro"
    )
    storage$theta_err_mat_5[, isim] <- theta_err_pofggm_5
    ...
    res_pofggm_4 <- compute_auc_graph_path_pofggm(
      theta_true_array = out_rPar$Tht,
      fit_list = pofggm_rho[[4]]
    )
    storage$auc_theta_vec_4[isim] <- res_pofggm_4$AUC_ROC
    
    res_pofggm_5 <- compute_auc_graph_path_pofggm(
      theta_true_array = out_rPar$Tht,
      fit_list = pofggm_rho[[5]]
    )
    storage$auc_theta_vec_5[isim] <- res_pofggm_5$AUC_ROC
    ...
    curve_err_pofggm <- sapply(pofggm_rho[[1]], function(fit) {
    ...
    curve_err_pofggm_2 <- sapply(pofggm_rho[[2]], function(fit) {
      compute_imputation_error_missing(
        X_hat = fit$Ximputed,
        X_true = X_full,
        X_obs = X_obs,
    ...
    curve_err_pofggm_3 <- sapply(pofggm_rho[[3]], function(fit) {
      compute_imputation_error_missing(
        X_hat = fit$Ximputed,
        X_true = X_full,
        X_obs = X_obs,
    ...
    curve_err_pofggm_4 <- sapply(pofggm_rho[[4]], function(fit) {
      compute_imputation_error_missing(
        X_hat = fit$Ximputed,
        X_true = X_full,
        X_obs = X_obs,
    ...
    storage$curve_err_mat_4[, isim] <- curve_err_pofggm_4
    
    curve_err_pofggm_5 <- sapply(pofggm_rho[[5]], function(fit) {
      compute_imputation_error_missing(
        X_hat = fit$Ximputed,
        X_true = X_full,
        X_obs = X_obs,
    ...
    storage$curve_err_mat_5[, isim] <- curve_err_pofggm_5
    ...
    if (isim %% 10L == 0L) {
      temp <- rbind(
        ...
        "ThetaError_POFGGM_4" = rowMeans(storage$theta_err_mat_4[, seq_len(isim), drop = FALSE], na.rm = TRUE),
        "ThetaError_POFGGM_5" = rowMeans(storage$theta_err_mat_5[, seq_len(isim), drop = FALSE], na.rm = TRUE),
        ...
        "AUC_POFGGM_4" = mean(storage$auc_theta_vec_4[seq_len(isim)], na.rm = TRUE),
        "AUC_POFGGM_5" = mean(storage$auc_theta_vec_5[seq_len(isim)], na.rm = TRUE),
        ...
        "CurveError_POFGGM_4" = rowMeans(storage$curve_err_mat_4[, seq_len(isim), drop = FALSE], na.rm = TRUE),
        "CurveError_POFGGM_5" = rowMeans(storage$curve_err_mat_5[, seq_len(isim), drop = FALSE], na.rm = TRUE),
        ...
        "ComputationalTime_POFGGM_4" = mean(storage$comp_time_vec_4[seq_len(isim)], na.rm = TRUE),
        "ComputationalTime_POFGGM_5" = mean(storage$comp_time_vec_5[seq_len(isim)], na.rm = TRUE)
      )
      print(round(temp[, seq(1, n_rho, by = 2L)], digits = 4L))
      save.image(paste0("~/Downloads/jcgs_simul_alpha_config", iconfig, ".Rdata"))
    }
  }
}
